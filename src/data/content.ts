...
type PageKey = "about" | "contact" | "privacy" | "terms" | "refund-policy";
...
export const pagesByLocale: Record<Locale, Record<PageKey, ContentPage>> = {
  en: {
    ...
    "refund-policy": { title: "Refund Policy", body: ["If you're not satisfied with your purchase, you have 30 days from delivery to return the product to us.", "Return shipping costs are the customer's responsibility. The item must be returned unworn, in its original condition, to the following address: address to be provided — please contact us before shipping a return.", "Once we receive and inspect your return, we'll issue a refund to your original payment method within 5–10 business days.", "Personalized (flocked) jerseys are final sale unless faulty."] },
  },
  fr: {
    ...
    "refund-policy": { title: "Politique de remboursement", body: ["Si vous n'êtes pas satisfait de votre achat, vous disposez de 30 jours à compter de la réception pour nous retourner le produit.", "Les frais de retour sont à votre charge. Le produit doit être renvoyé non porté et dans son état d'origine, à l'adresse suivante : adresse à venir — merci de nous contacter avant d'envoyer votre retour.", "Une fois le retour reçu et vérifié, nous procédons au remboursement sur votre moyen de paiement d'origine sous 5 à 10 jours ouvrés.", "Les maillots personnalisés (floqués) ne sont ni repris ni échangés, sauf en cas de défaut."] },
  },
  it: {
    ...
    "refund-policy": { title: "Politica di rimborso", body: ["Se non sei soddisfatto del tuo acquisto, hai 30 giorni dalla consegna per restituirci il prodotto.", "Le spese di reso sono a carico del cliente. L'articolo deve essere restituito non indossato e nelle condizioni originali, al seguente indirizzo: indirizzo da comunicare — ti preghiamo di contattarci prima di spedire il reso.", "Una volta ricevuto e controllato il reso, effettueremo il rimborso sul tuo metodo di pagamento originale entro 5–10 giorni lavorativi.", "Le maglie personalizzate sono vendita finale salvo difetti."] },
  },
  de: {
    ...
    "refund-policy": { title: "Rückerstattungsrichtlinie", body: ["Solltest du mit deinem Kauf nicht zufrieden sein, hast du ab Lieferung 30 Tage Zeit, um uns das Produkt zurückzusenden.", "Die Rücksendekosten trägt der Kunde. Der Artikel muss ungetragen und im Originalzustand an folgende Adresse zurückgesendet werden: Adresse folgt — bitte kontaktiere uns vor dem Versand einer Rücksendung.", "Nach Erhalt und Prüfung der Rücksendung erstatten wir den Betrag innerhalb von 5–10 Werktagen auf dein ursprüngliches Zahlungsmittel.", "Personalisierte (beflockte) Trikots sind vom Umtausch ausgeschlossen, außer bei Mängeln."] },
  },
  es: {
    ...
    "refund-policy": { title: "Política de reembolso", body: ["Si no estás satisfecho con tu compra, dispones de 30 días desde la entrega para devolvernos el producto.", "Los gastos de devolución corren a cargo del cliente. El artículo debe devolverse sin usar y en su estado original, a la siguiente dirección: dirección por confirmar — contáctanos antes de enviar tu devolución.", "Una vez recibida e inspeccionada la devolución, reembolsaremos tu método de pago original en un plazo de 5 a 10 días hábiles.", "Las camisetas personalizadas son venta final salvo defecto."] },
  },
  nl: {
    ...
    "refund-policy": { title: "Terugbetalingsbeleid", body: ["Als je niet tevreden bent met je aankoop, heb je vanaf levering 30 dagen om het product aan ons te retourneren.", "De retourkosten zijn voor rekening van de klant. Het artikel moet ongedragen en in originele staat worden geretourneerd naar het volgende adres: adres volgt — neem contact met ons op voordat je een retour verstuurt.", "Zodra we de retour hebben ontvangen en gecontroleerd, betalen we binnen 5–10 werkdagen terug op je oorspronkelijke betaalmethode.", "Gepersonaliseerde (bedrukte) shirts zijn uitgesloten van retour, tenzij defect."] },
  },
};
...
export const contentPageKeys: PageKey[] = ["about", "contact", "privacy", "terms", "refund-policy"];
